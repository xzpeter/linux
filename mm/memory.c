	       ...
	       unsigned long end)
{
	...
	do {
		next = pmd_addr_end(addr, end);
		if (is_swap_pmd(*src_pmd) || pmd_leaf(*src_pmd)) {
			int err;
				...
				struct zap_details *details)
{
	...
	do {
		next = pmd_addr_end(addr, end);
		if (is_swap_pmd(*pmd) || pmd_leaf(*pmd)) {
...
static inline void pfnmap_args_setup(struct follow_pfnmap_args *args,
				     ...
				     unsigned long pfn_mask, bool writable,
				     bool special)
{
	args->lock = lock;
	args->ptep = ptep;
	args->pfn = pfn_base + ((args->address & ~pfn_mask) >> PAGE_SHIFT);
	args->pgprot = pgprot;
