		...
		pud_t *pud, pfn_t pfn, bool write)
{
	...
	if (!pud_none(*pud)) {
		if (write) {
			WARN_ON_ONCE(pud_pfn(*pud) != pfn_t_to_pfn(pfn));
		...
		unsigned long haddr, bool freeze)
{
	...
	VM_BUG_ON(!is_pmd_migration_entry(*pmd) && !pmd_leaf(*pmd));
		...
		unsigned long address, bool freeze, struct folio *folio)
{
	...
	if (pmd_leaf(*pmd) || is_pmd_migration_entry(*pmd)) {
		/*
