...
static int hugetlb_vm_op_split(struct vm_area_struct *vma, unsigned long addr)
{
	...
	if (addr & ~PUD_MASK) {
		...
		if (floor < vma->vm_start || ceil >= vma->vm_end)
			...
			return 0;
		__hugetlb_unshare_pmds(vma, floor, ceil);
	}
						  ...
						  unsigned long reason)
{
	...
	struct vm_fault vmf = {
		...
		.real_address = addr,
		.flags = flags,
		/* Use small page address for HGM-minor enabled regions */
		.address =
		(hugetlb_hgm_advised(vma) && reason == VM_UFFD_MINOR) ?
		(addr & PAGE_MASK) : haddr,
	};
...
{
	__hugetlb_unshare_pmds(vma, ALIGN(vma->vm_start, PUD_SIZE),
