					 ...
					 unsigned long reason)
{
	 ...
	 * This must be a missing event, as we never handle wr-protect upon a
	 * special uffd-wp swap pte - it should first be converted into a
	 * normal read request before handling wp.  It just means the page/swap
	 * cache that backing this pte is gone, so this special pte is leftover.
	 *
	...
	if (pte_swp_uffd_wp_special(*pte)) {
		WARN_ON_ONCE(!(reason & VM_UFFD_MISSING));
		ret = true;
	}
				...
				unsigned long arg)
{
	...
	next:
		...
		/* Unshare all huge pmds if there is */
		if (is_vm_hugetlb_page(vma))
